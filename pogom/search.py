...
def login(auth,username,password, position):
    log.info('Attempting login to Pokemon Go.')

    api.set_position(*position)
    i=0
    while not api.login(auth, username, password):
        log.info('Failed to login to Pokemon Go. Trying again.')
        time.sleep(REQ_SLEEP)
        i=i+1
        if i==3:
            log.info("Too Many Bad Login Attempts")
            sys.exit()
    log.info('Login to Pokemon Go successful.')


def search(auth,username,password,location,steps):
    num_steps = int(steps)
    ...
    if api._auth_provider and api._auth_provider._ticket_expire:
        ...
        if remaining_time > 60:
            ...
            login(auth,username,password, position)
    else:
        login(auth,username,password, position)
...
def search_loop(auth,username,password,location,steps):
    while True:
        search(auth,username,password,location,steps)
